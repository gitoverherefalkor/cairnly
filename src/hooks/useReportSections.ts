...
export const useReportSections = (
  reportId?: string,
  opts?: { pollForExecSummary?: boolean },
) => {
  const { user } = useAuth();
  ...
  const {
    ...
    enabled: !!reportId,
    // The executive summary is generated by WF7, which is fired at chat
    // wrap-up and writes the exec_summary section a few seconds AFTER the
    // dashboard has already loaded. Without this, the UI only picks it up on
    // an incidental window-focus refetch (which can be minutes later). When
    // the caller asks us to, poll so the summary lands in the UI within ~15s.
    // This self-stops the moment the exec_summary section is present; the
    // caller flips the flag off after a timeout cap so a stuck WF7 can't poll
    // forever. (react-query pauses refetchInterval while the tab is
    // backgrounded, and refetchOnWindowFocus covers the return-to-tab case.)
    refetchInterval: opts?.pollForExecSummary
      ? (query) => {
          const data = query.state.data;
          const hasExec = !!data?.some(
            (s) =>
              s.section_type === 'exec_summary' ||
              s.section_type === 'executive_summary',
          );
          return hasExec ? false : 15000;
        }
      : false,
  });
