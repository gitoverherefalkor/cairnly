...
2. **No resume upload.** Skip the CV step entirely (Starter pattern in `src/pages/Assessment.tsx`). Their CV describes the life they are leaving; the survey asks about the one they want. Do not build the flavor-aware resume mapper.
3. **Fewer questions: target ~25, not 40.** Trim the section 2.4 draft accordingly. Keep structured widgets for structured facts (region, hours, ranking, constraints); make free-text prompts warm and conversational.
4. **Intake stays a structured survey, NOT a chat** (decided after a pros/cons review). The pipeline consumes `survey_responses` keyed by question UUIDs; conversational intake would need a transcript-to-structured extraction layer with new silent failure modes, worse widgets for ranking/dropdowns, awkward required-field enforcement, and harder autosave/resume. Instead, hybrid:
   - Conversational, voice-first free-text questions (the `long_text` mic input already exists; frame as "just talk, we'll type").
   - **Post-survey "story chat" (new, small):** after survey submit and before/while the report generates, a short 5-10 minute coach conversation (reuse chat-proxy/WF5 machinery) probes 2-3 stories based on their answers; append the transcript to the forward-to-n8n payload as supplementary evidence for WF1P (add a `story_transcript` field; WF1P prompt told to use it as evidence, tolerate its absence). If this proves heavy, ship v1 without it and fast-follow.
   - If full conversational intake is ever built, build it as a platform capability (all flavors; the chatbot-native argument is actually strongest for Starter), never an Encore-only fork.
...
3. ~~CV upload~~ **DECIDED: skip entirely** (see section 0).
4. **Section-2 depth:** with ~25 questions total (section 0), lean toward the lighter free-text career summary over the full career_history widget, compensated by the post-survey story chat. Confirm with Sjoerd if unsure.
