---
description: Continue Dutch localization. Reads docs/LOCALIZATION_PLAN.md, finds the next unchecked phase, executes through to the next stop-gate, then updates status.
---

You are continuing the Dutch localization rollout for Cairnly. The single source of truth is `docs/LOCALIZATION_PLAN.md`.

## Your job this session

1. **Read `docs/LOCALIZATION_PLAN.md` first.** Do not assume you know what's in it — it gets updated every session.
...
8. **Update `docs/LOCALIZATION_PLAN.md` Status section** before ending the session:
...
- **Never modify `docs/LOCALIZATION_PLAN.md` outside the Status section** without explicit user approval. The plan is the contract; status updates are bookkeeping.
...
Begin by reading `docs/LOCALIZATION_PLAN.md`.
