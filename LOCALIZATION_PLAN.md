...
- [ ] Phase 3 — Emails + static survey content (in progress)
  ...
  - [ ] **Emails** — in progress (per-language COPY tables, lang from preferred_language)
    ...
    - [x] `payment-success` (access-code receipt + referral-unlock) — deployed 2026-05-29. Buyer's receipt language threaded through Stripe checkout metadata: `CheckoutForm.tsx` sends `preferredLanguage: i18n.language` → `create-checkout` writes `metadata.preferred_language` → `payment-success` reads it (fallback: country=Netherlands→nl, else en). Referral-unlock email goes to a *different* person (the referrer), so it uses THAT user's `profiles.preferred_language`, not the buyer's metadata. Tool names (Find Open Roles etc.) kept English in both langs to match the then-still-English dashboard toolkit (the toolkit itself was localized on 2026-09-02, so these email strings are now the odd ones out and should follow); only prose + CTAs localized. Existing en em-dash in the unlock line replaced with a colon. All 3 files committed; both edge functions redeployed + smoke-tested (boot + validation paths green, no errors in logs).
  ...
  - [x] **Legal copy** (2026-05-29): Terms of Service, Privacy Policy, and Referral Terms now bilingual (EN+NL) via a markdown system. NL from Sjoerd's docx in public/terms, converted with scripts/docx-to-md.py to src/content/legal/<slug>.<lang>.md, rendered by src/components/legal/LegalDoc.tsx (react-markdown). New /referral-terms route. privacy-policy.en.md is a stopgap extracted from the old live English page; replace with the official EN docx when it arrives (re-run the converter). Security page left as-is (no docx). Build + deploy green.
- [ ] Phase 4 — n8n workflows (in progress, 2026-05-30)
  - **SCOPE EXPANDED (per user 2026-05-30):** live n8n audit found more user-facing workflows than the plan documented. Full scope is now WF1, WF3, WF4, WF5, WF_cover_letter (`M9w7xWeiPNmU7ZFb`), WF_custom_resume (`IFhL4Lno0hyMJ1Jc`), **WF7 ExecSummary (`yg7naUkC6oqr2WpU`, NEW)**, and the **feedback constellation (NEW)**: WF6 Feedback processing (`XuOb0iIv1Hwc2t62`), fb_strengths, fb_values, fb_development, fb_exec_sum, Top careers feedback, Outside Box feedback, Dream feedback, Runner feedback, Feedback Career Tool, archive_feedback_personality. WF2 = passthrough only (no user-facing text). User chose "do it all in one Phase 4". Revised effort: larger than the original 2-3 session estimate.
  - [x] **4.1 SOP addendum** — `n8n_aa/SOP_addendum_nl.txt` written (canonical Dutch output rules + inline instruction). All 6 core workflows backed up to `n8n_aa/backups_20260530/`.
  - [x] **4.2 WF5 Atlas Chat** (`h7ie9zN080IM2g7N`) — done 2026-05-30, **live**. Single edit to `Atlas Agent`: added `preferred_language` to SESSION DATA + appended OUTPUT LANGUAGE block. Workflow stayed active (n8n preserved active state on PUT). After-export saved. **Needs user test: Dutch chat returns Dutch.**
  - **Working on DUPLICATES (2026-05-31):** user created inactive duplicate workflows (EN/NL) so production stays untouched until swap-in. Edited the dups, not the live originals.
  - [x] **WF1 Profile Insert** dup (`0Z8WxV5tVFMJqIZt`) — threads `preferred_language` (webhook→Process Survey Data1→prompt + report_sections.language). Narrative prompt (`prompt_perso_prof1`) localized with EXACT Dutch subheaders; `init_summary` kept English (intermediate). `extract_report_sections_code1` made language-aware (EN+NL subheader patterns) + **no-match fallback so a section is never dropped** + writes `language` col. Passes `preferred_language` to WF2. **Tested END-TO-END with real LLM (Haiku 4.5) → parser: 5 sections, all nl, Dutch titles+subheaders, scores extracted, 0 dropped.**
  - [x] **WF3 scoring careers** dup (`zhgJuiDp60PS5ZKJ`) — only OOB is user-facing (scoring stays English). OOB prompt localized (Dutch subheaders Overzicht/Waarom dit bij je past/AI-impact op deze rol; AI-impact rating scale kept English). `Parse OOB` language-aware + `language` col. Lang from `Pull Profile Sections` (report_sections.language). Parser sim-tested EN+NL.
  - [x] **WF4 Career selection** dup (`seWmQPFQqIe60TkU`) — top-3 / runner-up / dream all localized. Added self-contained `Get Report Language` lookup node (report_sections by report_id) + rewire. 3 prompts get OUTPUT LANGUAGE (delimiters/metadata labels/```json``` kept English). `Parse runner up` rewritten language-agnostic (generic `## → <h5>`); Split Top3 + Parse Dream language-aware; all write `language`. All 3 parsers sim-tested EN+NL.
  - [x] **WF6 Feedback processing** dup (`CyyjL7D51NbVZNtL`) — AI returns JSON (no parsing coupling). Added `Get Report Language` side-branch off Parse Query; 5 Build-Prompt nodes get OUTPUT LANGUAGE instruction (match existing section structure/HTML). Update preserves existing `language`.
  - [x] **WF7 ExecSummary** dup (`ohNbCw7pVqvjCZHT`) — `Pull All Sections` now selects `language`; `Combine Sections` reads it; `Basic LLM Chain` localized; `Prepare for Insert` writes `language` + Dutch title ("Samenvatting").
  - All 5 dups **inactive** (PUT preserved inactive). Backups in `n8n_aa/dup_backups_20260530/`, after-exports in `n8n_aa/dup_after_20260530/`.
  - **⚠️ Open for user:** (a) the dups are NOT chained to each other — e.g. WF3-dup's `Execute WF4` still points to the ORIGINAL WF4. Decide swap-in wiring (each dup gets language independently via DB lookup, so they work standalone, but the chain must call the right WF for end-to-end). (b) WF_cover_letter + WF_custom_resume were NOT duplicated — still pending. (c) Activation is a user action.
